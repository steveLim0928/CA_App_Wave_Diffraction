...
def DS_Graph1 (canvas, a, z, colour):
    ...
    x6 = arange(168,254)#85
    def smallamplitude1(x):
        Fs = 425
        y = -2.5*(cos(2*pi*fr*(x-(168))/Fs))
        a.plot(y+2.5,x6,color=colour)
    smallamplitude1(x6)

    x7 = arange(253,344)#90
    def smallamplitude1(x):
        Fs= 450
        y = -4*(cos(2*pi*fr*(x-(253))/Fs))
        a.plot(y+4,x7,color=colour)
    smallamplitude1(x7)

    x8 = arange(343,429)#85
    def smallamplitude1(x):
        Fs = 425
        y = -2.5*(cos(2*pi*fr*(x-(343))/Fs))
        a.plot(y+2.5,x8,color=colour)
    smallamplitude1(x8)

    x9 = arange(428,499)#70
    def smallamplitude1(x):
        Fs = 350
        y = -1*(cos(2*pi*fr*(x-(428))/Fs))
